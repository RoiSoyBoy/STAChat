...
jest.mock("formidable", () => {
  return jest.fn(() => ({
    // Mocks the formidable constructor
...
describe("API Routes", () => {
  ...
  describe("/api/upload/pdf", () => {
    ...
    const mockPdfRequest = (
      ...
      if (fileData) {
        mockFormidableParse.mockImplementation(
          (_incomingMessage: any, callback: Function) => {
    ...
    it("should successfully ingest a PDF, process it, and store data", async () => {
      ...
      const mockFile: formidable.File = {
        ...
        // length: 1024,
        toJSON: function () {
          // Standard method for formidable.File
          return {
            size: this.size,
      ...
      mockFsReadFile.mockResolvedValueOnce(
        Buffer.from("mock PDF content buffer")
      );
      ...
      expect(mockChunkText).toHaveBeenCalledWith(
        "This is the extracted PDF text."
      );
      expect(mockClassifyTags).toHaveBeenCalledWith(
        "This is the extracted PDF text."
      );
      ...
      expect(mockUserUploadsCollection.doc).toHaveBeenCalledWith(
        expect.stringContaining("test-user-pdf-123-")
      );
